...
Usage:
    python -m kpub.classifier.scripts.train transformer --save
    python -m kpub.classifier.scripts.train embedding --table koa
    python -m kpub.classifier.scripts.train transformer --no-test --save
    python -m kpub.classifier.scripts.train transformer --year 2020-2024 --collection test_articles
...
Fine-tuning workflow:
    ...
    reviewed subset defined in src/classifier/config/article_subset.yaml:
    ...
    src/classifier/config/article_subset.yaml) to extend the window accordingly.
"""
...
# Local
from kpub.db_mongo_conn import from_config
from kpub.classifier.data.load_pubs import build_subset_query, load_pubs
from kpub.classifier.models.transformer import TransformerClassifier
from kpub.classifier.models.llm import LLMClassifier

PROJECT_ROOT = Path(__file__).parents[3]
CLASSIFIER_ROOT = Path(__file__).parents[1]
CONFIG_PATH = CLASSIFIER_ROOT / "config" / "models.yaml"
SUBSET_CONFIG_PATH = CLASSIFIER_ROOT / "config" / "article_subset.yaml"
OUTPUT_DIR = PROJECT_ROOT / "classifier_runtime" / "out" / "experiments"
...
if __name__ == "__main__":
    ...
    mongo_conn = from_config("kpub", args.collection)
